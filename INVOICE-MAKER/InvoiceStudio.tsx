...
export function InvoiceStudio({
  businessName = 'Lorem Ipsum Co.',
  businessEmail = 'lorem@ipsum.com',
  businessPhone = '+1 234 567 890',
  businessAddress = '123 Dolor Sit Amet\nConsectetur City, 12345',
  businessLogoUrl,
  taxName = '',
  taxNumber = '',
  clientName = 'Jane Doe',
  ...
  clientAddress = '456 Adipiscing Blvd\nElit Town, 67890',
  clientRef = '',
  ...
  dueDate,
  initialItems,
  paymentBank = 'Lorem Bank',
  paymentAccountName = 'Lorem Ipsum Co.',
  paymentAccountNumber = '1234-5678-9012',
  defaultNotes = 'Lorem ipsum dolor sit amet, consectetur adipiscing elit.',
  defaultTerms = 'Payment is due within 14 days of receiving this invoice.',
  initialTemplate = 'classic',
  initialColor = '#a78bfa',
  initialFont = 'Instrument Sans',
  initialDateFormat = 'MMM D, YYYY',
  initialPaid = false,
  initialDiscountAmt = 0,
  onDesignChange,
  onBack,
}: InvoiceStudioProps) {
  ...
  const [state, setState] = useState<InvoiceState>({
    ...
    issueDate: issueDate ?? today, dueDate: dueDate ?? dueDefault,
    ...
    items: initialItems ?? [
      { desc: 'Lorem ipsum dolor sit amet', sub: 'Consectetur adipiscing elit', qty: 2, price: 150 },
      { desc: 'Sed do eiusmod tempor', sub: '', qty: 1, price: 300 },
    ],
    discountPct: 0, discountAmt: initialDiscountAmt, taxName: 'Tax', taxPct: 0,
                ...
                {state.items.map((it, i) => (
                  <div key={i} style={{ background: T.bg, border: `1px solid ${T.border}`, borderRadius: 12, padding: 14, marginBottom: 8 }}>
                    ...
                    <Field label="Sub-description (optional)">
                      <Input value={it.sub} onChange={v => setItem(i, 'sub', v)} placeholder="e.g. additional details" style={{ color: T.textMuted, fontSize: 12.5 }} />
                    </Field>
